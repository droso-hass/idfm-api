from idfm_api import get_lines, get_stops, get_directions, get_traffic, get_infos
from idfm_api.models import TransportType

lines = get_lines(TransportType.METRO)
for i, val in enumerate(lines):
    print(f"#{i} - {val.name}")
line = lines[int(input("Select line "))]

print("\n")
stops = get_stops(line.id)
for i, val in enumerate(stops):
    print(f"#{i} - {val.name}")
stop = stops[int(input("Select stop area "))]

print("\n")
directions = get_directions(line.id)
...
print("\n")
print("Traffic:")
for i in get_traffic(line.id, stop.id, dir):
    print(f"Line {i.short_name} - Direction {i.direction}: {i.schedule}") 

print("\n")
print("Informations:")
for i in get_infos(line.id):
